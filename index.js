...
function isBotOwner(senderID) {
  return (config.bot.ownerIDs || []).includes(senderID);
}

function isBotAdmin(senderID) {
  return config.bot.adminIDs.includes(senderID) || isBotOwner(senderID);
}
...
async function handleMessage(api, event, commands) {
  ...
  try {
    await cmd.execute({ api, event: { ...event, isGroup }, args, commands, mutedThreads, lockedThreads, isOwner: isBotOwner(senderID) });
  } catch (e) {
